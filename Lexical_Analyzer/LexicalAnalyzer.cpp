...
int LexicalAnalyzer::getcharClass(char symbol)
{
	...
	if (symbol == EOF)
		Classchr = STOP;
	if (symbol == '+' || symbol == '-' || symbol == '*' || symbol == '<' || symbol == '>' || symbol == '&' || symbol == '.' || symbol == '@' || symbol == '/' || symbol == ':' || symbol == '=' || symbol == '~' || symbol == '|' || symbol == '!' || symbol == '#' || symbol == '%' || symbol == '^' || symbol == '_' || symbol == '[' || symbol == ']' || symbol == '{' || symbol == '}' || symbol == '"' || symbol == '`' || symbol == '?' || symbol == '\'' || symbol == '$')
		Classchr = OP_CODE;
	if (symbol == '(')
		Classchr = OPENING;
	if (symbol == ')')
	...
	case OP_CODE:
		...
		else if (charNext == '/')
		{
			...
			if (charNext == '/')
			{
				addCharLexeme();
				getChar();
				while (charNext != '\n' && charNext != '\r' && charNext != EOF && charNext != '!')
				{
